...
export function normalizeLatexForRender(text: string): string {
  ...
  const wrapNaked = (s: string): string => {
    let out = s;
    // One-level braces (common in quiz output)
    ...
    // Single-symbol commands (no braces) - wrap so KaTeX renders them
    out = out.replace(/\\(subseteq|subset|alpha|beta|gamma|delta|theta|lambda|pi|sigma|phi|omega|infty|leq|geq|neq|approx|pm|cdot|times|div|in|cup|cap|mid|sin|cos|tan|log|ln|quad|qquad|notin|emptyset|mathbf|mathbb)(?![a-zA-Z{])/g, '$$&$');
    return out;
  };
