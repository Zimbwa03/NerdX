// Virtual Lab API services (web)
// Ported from NerdX mobile app. Uses the same backend routes.
import api from './config';

export type VirtualLabSubject =
  | 'biology'
  | 'chemistry'
  | 'physics'
  | 'mathematics'
...
// Geography Maps Lab – map_actions for AI feedback
...
// Balance Sheet Lab – Vertex AI generated question
...
// Shared question shape for accounting labs
...
export const virtualLabApi = {
  getBalanceSheetQuestion: async (payload?: GetBalanceSheetQuestionPayload): Promise<BalanceSheetQuestion> => {
  ...
  getIncomeStatementQuestion: async (payload?: GetIncomeStatementQuestionPayload): Promise<IncomeStatementQuestion> => {
  ...
  getPartnershipAppropriationQuestion: async (payload?: GetAccountingLabQuestionPayload): Promise<AccountingLabQuestion> => {
  ...
  getCashFlowQuestion: async (payload?: GetAccountingLabQuestionPayload): Promise<AccountingLabQuestion> => {
    const response = await api.post<{ success: boolean; data: AccountingLabQuestion }>(
  ...
  getManufacturingAccountQuestion: async (payload?: GetAccountingLabQuestionPayload): Promise<AccountingLabQuestion> => {
    const response = await api.post<{ success: boolean; data: AccountingLabQuestion }>(
  ...
  getCorrectionOfErrorsQuestion: async (payload?: GetAccountingLabQuestionPayload): Promise<AccountingLabQuestion> => {
    const response = await api.post<{ success: boolean; data: AccountingLabQuestion }>(
  ...
  getNotForProfitQuestion: async (payload?: GetAccountingLabQuestionPayload): Promise<AccountingLabQuestion> => {
    const response = await api.post<{ success: boolean; data: AccountingLabQuestion }>(
  ...
  generateKnowledgeCheck: async (
    ...
    const response = await api.post('/api/mobile/virtual-lab/knowledge-check', payload, { timeout: 120000 });
    const data = (response.data as any)?.data || [];
    const credits_remaining = (response.data as any)?.credits_remaining;
    return { questions: Array.isArray(data) ? data : [], credits_remaining };
  },

  getGeoMapsFeedback: async (payload: GeoMapsFeedbackPayload): Promise<{ response: string; credits_remaining?: string }> => {
    const res = await api.post<{
      success: boolean;
      data: GeoMapsFeedbackResult;
      credits_remaining?: string;
    }>('/api/mobile/virtual-lab/geo-maps-feedback', payload, { timeout: 90000 });
    if (!res.data.success || !res.data.data) {
      throw new Error((res.data as any)?.message || 'Failed to get feedback');
    }
    return { response: res.data.data.response, credits_remaining: res.data.credits_remaining };
  },
};
