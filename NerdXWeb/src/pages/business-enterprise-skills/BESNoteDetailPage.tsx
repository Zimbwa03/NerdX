...
export function BESNoteDetailPage() {
  const { topicSlug } = useParams<{ topicSlug: string }>();
  const { user } = useAuth();

  const topicMeta = useMemo(() => {
    const topicId = (topicSlug || '').replace(/-/g, '_');
    return topicId ? getBESTopicById(topicId) : undefined;
  }, [topicSlug]);
  ...
  useEffect(() => {
    ...
    const topicNotes = besNotes[topicMeta.name as keyof typeof besNotes];
    if (topicNotes) {
      setNotes(topicNotes);
    } else {
      setNotes({
        ...
        summary: 'Notes for this topic are aligned with ZIMSEC O-Level Business Enterprise and Skills (4048).',
        sections: [
          {
            title: 'Overview',
            content: 'This topic is part of the ZIMSEC syllabus. Practice MCQs and essays are available for this topic.',
            diagrams: [],
            subsections: [],
          },
        ],
  ...
  if (loading) {
    return (
      <div className="science-universe-page bes loading-screen">
        ...
        <div className="loading-text">Loading BES Notes...</div>
      </div>
    );
  }
    ...
    return (
      <div className="science-universe-page bes">
        ...
        <div className="science-hero">
          ...
          <p>The requested BES notes could not be found.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="science-universe-page bes">
      ...
      <div className="science-notes-header-bar glass-panel">
        ...
        <div className="header-breadcrumbs">
          <span className="crumb-subject" style={{ color: ACCENT_COLOR }}>Business Enterprise and Skills</span>
