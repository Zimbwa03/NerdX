...
#!/usr/bin/env python3
"""
Credit management utility for NerdX Quiz Bot
...
def reset_specific_user_credits(user_id, new_credit_amount=1000):
    ...
    try:
        ...
        current_user = make_supabase_request("GET", "user_stats", select="credits", filters={"user_id": f"eq.{user_id}"})
        credits_before = current_user[0].get('credits', 0) if current_user else 0
        ...
        if result:
            ...
            transaction = {
                ...
                "credits_used": credits_before - new_credit_amount,  # Difference
            ...
            print(f"✅ User {user_id} credits reset from {credits_before} to {new_credit_amount}")
            return True
        else:
...
def add_credits_to_specific_user(user_id, credits_to_add, reason="manual_credit_addition"):
    """Add specific amount of credits to a single user"""
    try:
        success = add_credits(user_id, credits_to_add, reason, f"Manual credit addition: {credits_to_add} credits")
        
        if success:
            print(f"✅ Successfully added {credits_to_add} credits to user {user_id}")
            return True
        else:
...
def main():
    ...
    print("🛠️  NERDX CREDIT MANAGEMENT TOOL")
    print("=" * 50)
    ...
    while True:
        ...
        print("5. ➕ Add credits to specific user")
        print("6. 🚪 Exit")
        ...
        if choice == "1":
            ...
            user_id = input("Enter user ID: ").strip()
            amount = input("Enter credits to add: ").strip()
            try:
                amount = int(amount)
                add_credits_to_specific_user(user_id, amount)
            except ValueError:
                print("❌ Invalid amount")
