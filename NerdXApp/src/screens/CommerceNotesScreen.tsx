// Commerce Notes Screen – ZIMSEC O-Level Principles of Commerce
...
import { commerceTopics, CommerceTopic } from '../data/commerce';
...
const CommerceNotesScreen: React.FC = () => {
  const navigation = useNavigation();
  const { isDarkMode } = useTheme();
  const themedColors = useThemedColors();
  const [expandedTopic, setExpandedTopic] = useState<string | null>(null);

  const handleTopicPress = (topic: CommerceTopic) => {
    if (expandedTopic === topic.id) setExpandedTopic(null);
    else setExpandedTopic(topic.id);
  };

  const handleViewNotes = (topic: CommerceTopic) => {
    ...
    navigation.navigate('TopicNotesDetail' as never, {
      ...
      isALevel: false,
      topicData: topic,
      index: commerceTopics.indexOf(topic),
    } as never);
  };
  ...
  const getTopicIcon = (topicId: string): string => {
    const iconMap: Record<string, string> = {
      ...
      consumer_protection: 'shield-checkmark',
      business_organisations: 'business',
      enterprise: 'rocket',
      finance_and_banking: 'cash',
      insurance_and_assurance: 'umbrella',
      business_communication: 'chatbubbles',
      transport: 'car',
      warehousing: 'cube',
      marketing: 'megaphone',
    };
  ...
  return (
    <View style={[styles.container, { backgroundColor: themedColors.background.default }]}>
      ...
      <LinearGradient colors={commerceColors.primary} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.header}>
        <View style={styles.headerContent}>
          <View style={{ flex: 1 }}>
            <TouchableOpacity onPress={() => navigation.goBack()} style={styles.backButton}>
              <Text style={styles.backButtonText}>← Back</Text>
            </TouchableOpacity>
            <Text style={styles.title}>Commerce Notes</Text>
            <Text style={styles.subtitle}>ZIMSEC O-Level Principles of Commerce</Text>
          </View>
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        <View style={styles.topicsContainer}>
          ...
          {commerceTopics.map((topic, index) => (
            <View key={topic.id}>
              <Card variant="elevated" onPress={() => handleTopicPress(topic)} style={styles.topicCard}>
                <View style={styles.topicHeader}>
                  <View style={[styles.topicIcon, { backgroundColor: commerceColors.light }]}>
...
export default CommerceNotesScreen;
