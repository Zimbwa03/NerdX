// Not-for-Profit Income & Expenditure Lab – ZIMSEC O-Level Paper 2 Not-for-Profit Organizations
...
const FALLBACK_QUESTION: AccountingLabQuestion = {
  ...
  question_data: {
    trial_balance: [
      { account: 'Subscriptions received', debit: null, credit: 48000 },
      { account: 'Bar sales', debit: null, credit: 12000 },
      { account: 'Rent paid', debit: 18000, credit: null },
      { account: 'Wages', debit: 15000, credit: null },
      { account: 'Electricity', debit: 4000, credit: null },
      { account: 'Accumulated fund (1 Jan 2025)', debit: null, credit: 35000 },
      { account: 'Bar purchases', debit: 7000, credit: null },
      { account: 'Equipment (cost)', debit: 20000, credit: null },
      { account: 'Bank', debit: 21000, credit: null },
    ],
    adjustments: [
...
function normalizeQuestion(raw: any): AccountingLabQuestion {
  ...
  return {
    question_id: q.question_id || 'nfp-1',
    question_type: q.question_type || 'not_for_profit',
    difficulty_level: q.difficulty_level || 'intermediate',
    marks: q.marks,
    time_estimate: q.time_estimate,
    scenario: {
      business_name: scenario.business_name || 'Club',
...
const NotForProfitLabScreen: React.FC = () => {
  const navigation = useNavigation<any>();
  const themedColors = useThemedColors();
  const simulation = getSimulationById('accounting-not-for-profit-lab');
  ...
  const fetchQuestion = useCallback(async () => {
    ...
    setLoadError(null);
    try {
      const data = await virtualLabApi.getNotForProfitQuestion({ difficulty_level: 'intermediate', format: 'vertical' });
    ...
    <View style={[styles.container, { backgroundColor: themedColors.background.default }]}>
      ...
      {loading ? (
        <View style={styles.loadingWrap}>
          ...
          <Text style={[styles.loadingText, { color: themedColors.text.secondary }]}>Generating your Not-for-Profit question…</Text>
        ...
        <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent}>
          ...
          <TouchableOpacity style={[styles.card, { backgroundColor: themedColors.background.paper }]} onPress={() => setShowTrialBalance(!showTrialBalance)} activeOpacity={0.8}>
            <View style={[styles.cardAccent, { backgroundColor: ACCENT + '20' }]}>
              ...
              <Text style={[styles.cardTitle, { color: themedColors.text.primary }]}>Trial Balance / Receipts & Payments (extract)</Text>
          ...
          <TouchableOpacity style={[styles.card, { backgroundColor: themedColors.background.paper }]} onPress={() => setShowModelAnswer(!showModelAnswer)} activeOpacity={0.8}>
            <View style={[styles.cardAccent, { backgroundColor: ACCENT + '20' }]}>
              ...
              <Text style={[styles.cardTitle, { color: themedColors.text.primary }]}>Model Income & Expenditure (check your answer)</Text>
            ...
            {showModelAnswer && summary && (
              <View style={styles.modelAnswer}>
                {summary.subscription_income != null && <Text style={[styles.modelSection, { color: themedColors.text.primary }]}>Subscription income: ${(summary.subscription_income as number).toLocaleString()}</Text>}
                {summary.total_income != null && <Text style={[styles.modelSection, { color: themedColors.text.primary }]}>Total income: ${(summary.total_income as number).toLocaleString()}</Text>}
                {summary.total_expenditure != null && <Text style={[styles.modelSection, { color: themedColors.text.primary }]}>Total expenditure: ${(summary.total_expenditure as number).toLocaleString()}</Text>}
                {summary.surplus_or_deficit != null && <Text style={[styles.modelSection, { color: ACCENT }]}>Surplus: ${(summary.surplus_or_deficit as number).toLocaleString()}</Text>}
                {summary.accumulated_fund_balance != null && <Text style={[styles.modelSection, { color: ACCENT }]}>Accumulated fund balance: ${(summary.accumulated_fund_balance as number).toLocaleString()}</Text>}
              </View>
            )}
          </TouchableOpacity>
...
export default NotForProfitLabScreen;
