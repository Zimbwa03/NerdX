#!/usr/bin/env python3
"""
Direct database operations for the Telegram bot
This module handles all database interactions without async/await
"""
...
def add_referral_credits(referred_by_nerdx_id, new_user_chat_id):
    ...
    try:
        ...
        # Add referral bonus credits using centralized config
        ...
        if success:
            ...
            # Optionally notify the referrer about the credit bonus
            try:
                from whatsapp_handler import WhatsAppHandler
                whatsapp = WhatsAppHandler()

                notification_message = f"🎉 *Referral Bonus!* 🎉\n\n"
                notification_message += f"👤 **{new_user_name}** just joined NerdX using your referral code!\n\n"
                notification_message += f"💰 **+{referral_bonus} Credits** added to your account\n"
                notification_message += f"🚀 Keep sharing to earn more!"

                whatsapp.send_message(referrer_chat_id, notification_message)
                logger.info(f"Sent referral notification to {referrer_chat_id}")

            except Exception as notification_error:
                logger.error(f"Failed to send referral notification: {notification_error}")
