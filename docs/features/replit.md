...
## UI/UX Decisions
The web dashboard uses Bootstrap 5 with a dark theme, Font Awesome for icons, and custom CSS. Comprehension passages are converted to professional, ZIMSEC-branded PDF documents, and LaTeX expressions are converted to readable Unicode. Custom MediaPlayer component (NerdXWeb/src/components/MediaPlayer.tsx) provides premium video and audio players with custom controls (play/pause, progress, volume, speed, fullscreen, skip). VideoPlayer auto-detects YouTube/embed URLs and renders iframe; direct MP4 URLs get full custom player. AudioPlayer features disc animation, progress bar, and speed cycling. Used across ScienceNotesPage, MathNotesPage, and AccountingNoteDetailPage.
