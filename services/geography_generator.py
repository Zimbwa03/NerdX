...
- Optional consumer Gemini API as FALLBACK when Vertex fails or is unavailable.
...
class GeographyGenerator:
    ...
    def generate_structured_question(
        ...
        try:
            ...
            response = try_gemini_json(vertex_prompt, logger=logger, context=context)
            if response:
                return self._validate_and_enhance_structured(response, topic, difficulty, user_id, source="gemini_fallback")
